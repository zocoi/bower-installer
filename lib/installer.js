...
function installFile(f, key, paths, dep, callback) {
  ...
  var f_s,  // The path for the file to be moved
    ...
    if(!f_name_new) {
        ...
        if (last && typeof last[1] !== undefined) {
            ...
            subDir = (f_name_helper[1] != '')
            ? pathLib.dirname(f_name_helper[1])
            : f_name_helper[1];
        }
    }
    ...
    if( paths.all ) {
        ...
        path = paths[utils.getExtension(f_name)] +  pathSep + key + subDir;

        mkdirp.sync(
            pathLib.normalize(basePath + pathSep + path + pathSep + subDir),
            0755
        );
    }
...
exports.installDependency = function installDependency(deps, key, cfg, paths, callback) {
    var base, other;
    ...
    async.each(deps, function(dep, callback) {
      ...
      // the file
      if(_.isObject(dep)) {
        installFile(dep, key, paths, dep, callback);
      } else {
        glob(dep, function(err, files) {
          if(err) {
            ...
            async.each(files, function(f, callback) {
              installFile(f, key, paths, dep, callback);
            }, callback);
          }
        });
      }
