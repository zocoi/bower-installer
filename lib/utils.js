...
exports.parsePath = function(path, map, pathSep) {
    if (path.indexOf('{') !== -1) {
        ...
        _.each(keys, function(key) {
          ...
          // only name and version are supported now
          if (key in map) {
            path = path.replace(new RegExp('\\{[ ]?' + key + '[ ]?\\}', 'gi'), map[key]);
          }

        });

    } else {
        path += pathSep + map.name;
    }
    return path;
};

exports.getJSON = function(path, pathSep) {
  var pakcfg = null;

    // If it is a directory lets try to read from package.json file
  if (fs.lstatSync(path).isDirectory()) {
    var pakpath = path + pathSep + "bower.json";
